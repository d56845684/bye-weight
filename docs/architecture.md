...
#### JWT 設計（IAM-style，精簡到最小 identity）

```json
// Access Token payload
{
    "user_id":   42,
    "role":      "patient",      // patient / staff / nutritionist / admin / super_admin
    "tenant_id": 3,              // 0 = system tenant（super_admin 專用）
    "jti":       "uuid-xxxx",    // 唯一 ID，用於 blacklist 撤銷
    "exp":       1700000000,     // Access Token：15 分鐘
    "iat":       1699999100,
    "type":      "access"
}
...
**重點**：領域欄位（patient_id / clinic_id）**不再出現於 JWT**。下游服務透過 `X-User-Id + X-Tenant-Id` 自行查身份映射表（例：`main_service.patients.auth_user_id`）。
...
### 7A.4 auth_db Schema（IAM-style + 多租戶 Hard isolation）

```sql
-- ── 租戶（Hard isolation；id=0 是 system tenant，super_admin 專用）──
CREATE TABLE tenants (
    id         INT PRIMARY KEY,                  -- 固定 0 給 system；其他自增
    slug       VARCHAR(50) UNIQUE NOT NULL,      -- clinic-a / hospital-b
    name       VARCHAR(100) NOT NULL,
    active     BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT NOW()
);

-- ── 使用者（只留 identity；領域欄位都拿掉）──
CREATE TABLE users (
    id            SERIAL PRIMARY KEY,
...
-- ── 角色（目前全域共用；Soft isolation 升級時再加 tenant_id nullable）──
CREATE TABLE roles (
    id   SERIAL PRIMARY KEY,
    name VARCHAR(50) UNIQUE NOT NULL   -- patient/staff/nutritionist/admin/super_admin
);

-- ── 服務註冊（每個下游服務登記一筆：main / auth / billing…）──
CREATE TABLE services (
    id     SERIAL PRIMARY KEY,
    name   VARCHAR(50) UNIQUE NOT NULL,  -- main / auth / billing
    prefix VARCHAR(100) NOT NULL          -- /api / /auth
);

-- ── 動作映射：HTTP 請求 → (action, resource ARN template)
-- resource_template 支援 ${auth:user_id} / ${auth:tenant_id} / ${path.{name}}
CREATE TABLE action_mappings (
    id                SERIAL PRIMARY KEY,
    service_id        INT REFERENCES services(id) ON DELETE CASCADE,
    http_method       VARCHAR(10) NOT NULL,
    url_pattern       VARCHAR(200) NOT NULL,       -- /visits/{id}/medications
    action            VARCHAR(100) NOT NULL,        -- main:visit:read
    resource_template VARCHAR(300) NOT NULL,        -- main:tenant/${auth:tenant_id}/user/${auth:user_id}/visit/${path.id}
    UNIQUE (service_id, http_method, url_pattern)
);

-- ── Policy Document（AWS IAM 風格；JSONB 文件直接存）──
...
-- ── 角色 ↔ Policy 綁定（一個 role 可掛多份 policy）──
...
**預設 5 份 policy**（migration `000008_iam_policies` seed）：
...
> **⚠️ 下方程式碼為初版 RBAC+PBAC 設計的範例。目前實際實作已切換為 IAM-style（詳見 §7A.4）**。核心差異：
> - `Subject` / `Resource` 改為 `tenant_id + user_id + role` 三欄位
> - `engine.ResolveAction(method, uri) → (action, resourceTemplate, pathAttrs)` 取代 `ResolvePermission`
> - `engine.Check(sub, action, resource)` 用 `${auth:*}` 變數替換 + glob 匹配 JSONB policy document，遵循 **explicit deny > allow > implicit deny**
> - `handler/verify.go` 只注入 `X-User-Id / X-User-Role / X-Tenant-Id`（不再寫 X-Patient-Id / X-Clinic-Id）
>
> 最新實作直接看 `auth_service/engine/engine.go` + `handler/verify.go`。
...
### 7A.7 Main Service 的改動（Python FastAPI）

主服務完全不處理授權判斷，從 Nginx 注入的三個 generic header 讀身份，再查自己的業務表解析 profile：
...
async def current_patient(
    ...
    """從 auth_user_id + tenant_id 查出當前 user 的 patient profile。
    defence-in-depth：同時濾 tenant_id 確保跨租戶 URL 偽造也擋不過。"""
...
# 路由：auth_service 已經確保 role+policy 有權存取；這裡只做身份映射 + tenant 防線
...
async def inbody_history(
    ...
    stmt = select(InbodyRecord).where(
...
**關鍵改動**：
- `X-Clinic-Id / X-Patient-Id` 移除，改為 `X-Tenant-Id`
- `current_patient` dependency 用 `auth_user_id UNIQUE` 查出 profile（一個 auth user 對一個 patient）
- 每個 query 都帶 `tenant_id == user["tenant_id"]` 雙保險
